...
const scrapeContent = async () => {
    ...
    for (const course of data) {
        const { title, summary, Level, subject, course_description, course_url } = course;
        ...
        if (!headerImageUrl) {
            ...
            console.log(`Video Thumb Image URL for ${course_url}: ${videoThumbUrl}`);

            try {
                await page.waitForSelector('.video-trigger-cta', { timeout: 3000 });
                await page.click('.video-trigger-cta'); 
                ...
                console.error(`Error clicking video trigger for ${course_url}: ${error.message}`);
            }
        }

     
        const query = 'INSERT INTO articles (title, description, difficulty, tags, summary, image_url, video_url) VALUES ($1, $2, $3, $4, $5, $6, $7) RETURNING id';
        const values = [title, course_description, Level, subject, summary, headerImageUrl || null, videoSrc || null];
